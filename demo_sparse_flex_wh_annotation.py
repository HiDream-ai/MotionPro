...
output_dir = "all_results/test/annotation"
...
def visualize_drag_v2(background_image_path, splited_tracks, width, height, mask):
    trajectory_maps = []
    ...
    mask_area = mask[0].squeeze().numpy()
    transparent_background[mask_area,-1] = 255
...
class Drag:
    ...
    def run(self, first_frame_path, tracking_points, inference_batch_size, motion_bucket_id, first_frame_path_mask=None, img_ratio=None, image_label_name=None):
        ...
        mask_b = mask.to(input_drag.dtype)      #  convert into float ,match with training..  # ! NOTE
        input_drag = torch.concat([input_drag, mask_b], dim=-1)           # todo_for add frame1: change shape [b,h,w,3] -> [b,h,w,6]
        ...
        visualized_drag, _ = visualize_drag_v2(first_frame_path, resized_all_points, self.width, self.height, mask)
        ...
        for i in range(inference_batch_size):
            ...
            outputs_path = os.path.join(output_dir, 'gifs', f'output_{i}_{id}.gif')
        ...
        # NEW ADD save all annotations..
        ann_save_path = os.path.join(output_dir, image_label_name)
        os.makedirs(ann_save_path, exist_ok=True)
        shutil.copy(first_frame_path, os.path.join(ann_save_path, 'first_frame.png'))
        shutil.copy(first_frame_path_mask, os.path.join(ann_save_path, 'first_frame_mask.png'))
        json_dict = {'traj_key_points': resized_all_points}
        save_dict_to_json(json_dict, os.path.join(ann_save_path, 'meta_data.json'))
        
        user_drag = convert_rgba_to_rgb(visualized_drag[0])
        user_drag.save(os.path.join(ann_save_path, 'user_drag.png'), format='PNG')
...
with gr.Blocks() as demo:
    gr.Markdown("""
        ...
        <h2 align='center'>CVPR 2025 Paper Demo</h2><br>
    """, unsafe_allow_html=True)
    gr.Markdown(
        """
        ## How to Use MotionPro:<br>
        1. **Upload an Image** - Click to select an image and load it into the input box.<br>
        2. **Brush a Mask** - Use the brush tool to create a mask on the image.<br>
        3. **Initialize Mask & Crop** - Click **"Init mask and crop img"** to process the mask and crop the image.<br>
        4. **Define Motion Paths**:<br>
            - Click **"Add Drag"** to create a new trajectory path.<br>
            - Click multiple points to form a motion trajectory.<br>
            - Click **"Delete last drag"** to remove the last entire trajectory path.<br>
            - Click **"Delete last step"** to remove the most recent point from the trajectory.<br>
        5. **Run Animation** - Click **"Run"** to animate the image along the defined path.<br>
    ...
    def preprocess_image(tracking_points, imageMask):
        ...
        id = str(uuid.uuid4())[:4]
        first_frame_path = os.path.join(output_dir, 'image_log', 'first_frame', f"first_frame_id_{id}.png")
        ...
        first_frame_path_mask = os.path.join(output_dir, 'image_log', 'masks', f"first_frame_id_{id}_mask.png")
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            motion_bucket_id = gr.Slider(label='Motion Bucket',
                                             ...
                                             step=1, 
                                             value=17)
            image_label_name = gr.Textbox(label = "image label name")
        with gr.Column(scale=4):
            with gr.Row():
                with gr.Column(scale=1):
    ...
    delete_last_drag_button.click(delete_last_drag, [tracking_points, first_frame_path], [tracking_points, input_image])   # convert input_image into imageMask
    ...
    run_button.click(MotionPro.run, [first_frame_path, tracking_points, inference_batch_size, motion_bucket_id, first_frame_path_mask, img_ratio, image_label_name], [output_image, output_video])
