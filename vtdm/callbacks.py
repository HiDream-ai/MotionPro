...
from vtdm.utils import export_to_video
...
class ImageLogger(Callback):
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (self.check_frequency(check_idx) and  # batch_idx % self.batch_freq == 0
            ...
            with torch.no_grad():                       # the former codes don't neeed change
                images = pl_module.log_images(batch, split=split, **self.log_images_kwargs)


            batch_masked_flow = images['flow'].permute(0,1,3,4,2)[:,:,:,:,:2]
            batch_mask = images['flow'].permute(0,1,3,4,2)[:,:,:,:,-1]
            # batch_mask = batch['flow_brush_mask'].squeeze()
            b,f,h,w = batch_mask.shape
            
            node_rank = int(os.environ.get('RANK'))
            base_save_dir = os.path.join(pl_module.logger.save_dir, 'image_log')
            filename = "results_gs-{:06}_e-{:06}_b-{:06}".format(pl_module.global_step, pl_module.current_epoch, batch_idx)
            ...
            for i in range(b):
                ...
                for j in range(f):
                    ...
                    masked_flow = flow_to_image(batch_masked_flow[i,j].unsqueeze(0).permute(0, 3, 1, 2)).permute(0, 2, 3, 1).squeeze().detach().cpu()
                    mask = (batch_mask[i,j]*255).to(torch.uint8).unsqueeze(-1).repeat(1,1,3).detach().cpu()
                    
                    masked_arrow = torch.from_numpy(
                        attach_arrows(batch_masked_flow[i,j].detach().cpu().numpy(), base_save_dir, pl_module.global_step, node_rank, idx=j)
                        ).to(torch.uint8)
                ...
                video_name = os.path.basename(batch['video_name'][i]) + '.mp4'
                video_path = os.path.join(save_video_dir, video_name)
                
                torchvision.io.write_video(video_path, concat_video, fps=8, video_codec='h264', options={'crf': '16'})
...
class TextProgressBar(pl.callbacks.ProgressBarBase):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        ...
        if self._should_update(current, self.total_train_batches):
            ...
            msg = f'[Epoch {trainer.current_epoch}] [Batch {self.train_batch_idx}/{self.total_train_batches} {batch_time:.2f} s/batch] [{(self.total_train_batches*batch_time/3600):.1f} h/epoch]  => '
