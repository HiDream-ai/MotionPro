...
def watermark_inpaint(x: torch.Tensor, mask: np.array, radius: int = 3) -> torch.Tensor:
    assert len(x.shape) == 4
    ...
    img_array = np.concatenate(
        [np.expand_dims(img_array, axis=0) for img_array in inpaint_img_array_list],
        axis=0,
    )
    img_tensor = torch.from_numpy(np.ascontiguousarray(img_array))
    img_tensor = img_tensor.permute(3, 0, 1, 2)
...
def find_offset(img1: np.array, img2: np.array):
	...
	img3 = cv2.matchTemplate(img2,img1,cv2.TM_CCORR_NORMED)
...
def move_box(alpha: np.array, W: np.array, offset: list):
	pos = alpha.nonzero()
	box = [pos[0].min(), pos[1].min(), pos[0].max(), pos[1].max()]
...
def detect_remove_watermark(image: np.array, alpha: np.array, W: np.array, bg: np.array):
	
	offset = find_offset(W, image)
	...
	image[ROI] = I
	fI = I.copy()
	fI = cv2.medianBlur(fI.astype(np.uint8), 3)
	I[med!=0] = fI[med!=0]
	I[I<0] = 0
	I[I>255] = 255
	image[ROI] = I
...
def watermark_remove(x: torch.Tensor, alpha: np.array, W: np.array, bg: np.array) -> torch.Tensor:
    assert len(x.shape) == 4
    assert x.dtype == torch.uint8
    c, t, h, w = x.shape
    if h == 336 and w == 596 and c == 3:
        watermark_remove_img_list = []
        img_array_list = [
            img_tensor.squeeze(0).numpy()
            for img_tensor in x.permute(1, 2, 3, 0).split(1, dim=0)
        ]
        for img in img_array_list:
            ...
            img_res = detect_remove_watermark(img_bgr, alpha, W, bg)
            watermark_remove_img_list.append(img_res[...,::-1]) # convert BGR to RGB
        img_array = np.concatenate(
            [np.expand_dims(img_array, axis=0) for img_array in watermark_remove_img_list],
            axis=0,
        )
        img_tensor = torch.from_numpy(np.ascontiguousarray(img_array))
        img_tensor = img_tensor.permute(3, 0, 1, 2)
...
# upgrade watermark removal

def find_offset_v2(img1: np.array, img2: np.array):
    ...
    box2 = [box[0]-5, box[1]-5, box[2]+5, box[3]+5]
    idx2 = np.ix_(range(box2[0],box2[2]), range(box2[1], box2[3]))
    img2 = img2[idx2]
    img2 = cv2.cvtColor(img2.astype(np.uint8), cv2.COLOR_BGR2GRAY)
    
    img3 = cv2.matchTemplate(img2,img1,cv2.TM_CCORR_NORMED)	
    # img3 = cv2.matchTemplate(img2,img1,cv2.TM_SQDIFF_NORMED)		
    cent = [img3.shape[0]//2, img3.shape[1]//2]
    img3 = img3[cent[0]-5:cent[0]+5, cent[1]-5:cent[1]+5]
    maxindex = img3.argmax()
    row, col = maxindex//img3.shape[1], maxindex%img3.shape[1]
    assert img3[row,col]==img3.max()
    
    return [5-row,5-col]        # ! change


def move_box_v2(alpha: np.array, W: np.array, edge_image: np.array, offset: list):
	pos = alpha.nonzero()
	box = [pos[0].min(), pos[1].min(), pos[0].max(), pos[1].max()]
	new_alpha = np.zeros(alpha.shape, dtype=alpha.dtype)
	new_W =  np.zeros(alpha.shape, dtype=W.dtype)
	new_edge_image = np.zeros(edge_image.shape, dtype=edge_image.dtype)
	...
	new_alpha[idx2] = alpha[idx1]
	new_W[idx2] = W[idx1]
	new_edge_image[idx2] = edge_image[idx1]
	return new_alpha, new_W, edge_image		# ! change


def detect_remove_watermark_v2(input_images, alpha, W, edge_image=None):
    offset = find_offset_v2(W, input_images)
    alpha, W, edge_image = move_box_v2(alpha, W, edge_image, offset)
    
    # 1、subtract watermark (1)res_img: I 、 (2)filter_all_img: fI
    frame = input_images
    I = (frame.astype(np.float32) - alpha * W)/(1-alpha)
    I[I>255] = frame[I>255]
    I[I<0] = frame[I<0]
    I[I>255]=255
    I[I<0] = 0
    ...
    I = I.astype(np.uint8)
    fI = I.copy()
    fI = cv2.medianBlur(fI, 3)

    # 2、filt watermark1
    mask0 = edge_image!=0
    mask = mask0 | np.roll(mask0, 2, axis=1) | np.roll(mask0, -2, axis=1) | np.roll(mask0, 2, axis=0) | np.roll(mask0, -2, axis=0)
    bfI = I.copy()
    bfI[mask] = fI[mask]

    # 3、filt watermark2
    bfI_f = bfI.copy()
    bfI = cv2.medianBlur(bfI, 7)
    bfI_f[mask0] = bfI[mask0]
    
    return bfI_f


def watermark_remove_v2(x: torch.Tensor, alpha: np.array, W: np.array, edge_image: np.array) -> torch.Tensor:
    assert len(x.shape) == 4
    assert x.dtype == torch.uint8
    c, t, h, w = x.shape
    if h == 336 and w == 596 and c == 3:
        ...
        for img in img_array_list:
            img_bgr = img[...,::-1] # convert RGB to BGR
            img_res = detect_remove_watermark_v2(img_bgr, alpha, W, edge_image)
            watermark_remove_img_list.append(img_res[...,::-1]) # convert BGR to RGB
        img_array = np.concatenate(
            [np.expand_dims(img_array, axis=0) for img_array in watermark_remove_img_list],
            axis=0,
