...
if __name__ == "__main__":
    ...
    # *-------- 1. Retrieve parameters from configuration files 
    # - And define the base directory for logging.
    ...
    # *--------- 2. Initialize logging and setup
    #  - Create pl_logger (text file) for logging
    #  - Save core components code (sgm, vtdm, dataset code)
    #  - Copy config files for model and training
    ...
    try:
        ...
        # *--------- 4. Define trainer and callbacks functions
        
        # For instantiating the trainer and callbacks.
        trainer_kwargs = dict()             
        ...
        modelckpt_cfg = OmegaConf.merge(default_modelckpt_cfg, modelckpt_cfg)           # define callback_2: ModelCheckpoint
        ...
        if version.parse(pl.__version__) >= version.parse('1.4.0'):
            default_callbacks_cfg.update({'checkpoint_callback': modelckpt_cfg})            # define callback_3,4,5:

        if "callbacks" in lightning_config:
            callbacks_cfg = lightning_config.callbacks                                      #  get specific config
        else:
        ...
        callbacks_cfg = OmegaConf.merge(default_callbacks_cfg, callbacks_cfg)    # NOTE: if not specify in callback_cfg, use default_callback_cfg
